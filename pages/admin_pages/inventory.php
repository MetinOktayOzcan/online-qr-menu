...
<div class="container-fluid pt-4 px-4">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="admin?sayfa=inventoryAdd" class="btn btn-warning fw-bold text-dark">
                    ...
                    <tr>
                        <td>
                            <div class="bg-secondary rounded align-items-center justify-content-center d-flex text-white mb-2 overflow-hidden" style="width: 60px; height: 60px;">
                                <?php 
                                    $dosya_adi = basename($row['resim_yolu']);
                                    $fiziksel_yol = "uploads/" . $dosya_adi;
                                ?>

                                <?php if (!empty($dosya_adi) && file_exists($fiziksel_yol)): ?>
                                    <img src="<?= ROOT ?>/uploads/<?= $dosya_adi ?>" 
                                        alt="Ürün Resmi" 
                        ...
                        <td class="text-end">
                            <a href="admin?sayfa=inventoryEdit&id=<?php echo $row['id']?>" class="btn btn-sm btn-outline-info border-0 text-info me-2">
                                <span class="material-symbols-outlined fs-5">edit</span>
                            </a>
                            <a href="admin?sayfa=inventoryDelete&id=<?php echo $row['id']?>" class="btn btn-sm btn-outline-danger border-0 text-danger" onclick="return confirm('Silmek istediğine emin misin?')">
